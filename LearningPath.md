...
```mermaid
graph TD
    U[사용자 프로필<br>(선호하는 장르, 키워드)] -->|매칭| R[추천 아이템]
    I[아이템 특징<br>(장르, 설명)] -->|유사도 계산| S[유사 아이템]
    S --> R
```
...
graph TD
    ...
    Hidden2 --> Output[출력 계층<br>(상호작용 예측)]
```
...
graph TD
    ...
    U_embed -->|유사도 계산<br>(예: 내적)| Prediction[예측/추천]
    I_embed -->|유사도 계산<br>(예: 내적)| Prediction
```
