...
def test_run_item_cf_example():
    """
    Test that the item_cf_example.py script runs successfully and produces output.
    """
    script_path_rel = "examples/collaborative_filtering/item_cf_example.py"
    ...
    env = os.environ.copy()
    # env["TF_CPP_MIN_LOG_LEVEL"] = "2" # Not a TF script, but kept for consistency example
...
def test_run_user_cf_example():
    """
    Test that the user_cf_example.py script runs successfully and produces output.
    """
    script_path_rel = "examples/collaborative_filtering/user_cf_example.py"
    script_path_abs = os.path.join(project_root, script_path_rel)
    assert os.path.exists(script_path_abs), f"Script not found: {script_path_abs}"
    ...
    env = os.environ.copy()
    # env["TF_CPP_MIN_LOG_LEVEL"] = "2" # Not a TF script
