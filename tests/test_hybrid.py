...
def test_run_two_tower_hybrid_example():
    """
    Test that the two_tower_hybrid_example.py script runs successfully and produces output.
    """
    script_path_rel = "examples/hybrid/two_tower_hybrid_example.py"
    ...
    generic_keywords_present = (
        "추천된 아이템" in process.stdout or
        "recommended items" in process.stdout.lower() or
        "유사도 점수" in process.stdout or
        "similarity score" in process.stdout.lower() or
        "추천 아이템 목록" in process.stdout
    )
